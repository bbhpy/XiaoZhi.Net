...
#include <esp_log.h>
#include <utility>
#include <esp_netif.h>        // 添加这个
#include <esp_netif_types.h>  // 添加这个
...
WifiBoard::WifiBoard() {
    ...
    esp_timer_create(&timer_args, &connect_timer_);
        // 创建IPv6检查定时器（先创建，但不启动）
    esp_timer_create_args_t ipv6_timer_args = {
        .callback = OnIPv6CheckTimer,
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "ipv6_check_timer",
        .skip_unhandled_events = true
    };
    esp_timer_create(&ipv6_timer_args, &ipv6_check_timer_);
}
...
WifiBoard::~WifiBoard() {
    ...
    if (ipv6_check_timer_) {
        esp_timer_stop(ipv6_check_timer_);
        esp_timer_delete(ipv6_check_timer_);
    }
}
...
void WifiBoard::ConfigureIPv6() {
    esp_netif_t* netif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    if (netif == NULL) {
        ESP_LOGW(TAG, "WiFi station netif not found");
        return;
    }
    
    // 创建IPv6链路本地地址
    esp_err_t ret = esp_netif_create_ip6_linklocal(netif);
    if (ret == ESP_OK) {
        ...
        if (esp_netif_get_ip6_linklocal(netif, &ip6_addr) == ESP_OK) {
            PrintIPv6Address(ip6_addr, "link-local");
        }
    } else {
        ESP_LOGW(TAG, "Failed to create IPv6 link-local address: %s", esp_err_to_name(ret));
    }
    
    ESP_LOGI(TAG, "IPv6 configured, waiting for global address from router advertisement");
}
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
   ...
   // Stop timeout timer
            ...
            // 添加这行：WiFi连接成功后，重新配置IPv6
            ConfigureIPv6();
            
            // 重置IPv6状态标志（如果需要）
            ipv6_global_obtained_ = false;
            ipv6_link_local_printed_ = false;
        ...
        case NetworkEvent::Disconnected:
            ...
            ipv6_global_obtained_ = false;
            ipv6_link_local_printed_ = false;  // 重置链路本地地址打印标志
            StopIPv6Monitoring();
            break;
...
void WifiBoard::OnIPv6CheckTimer(void* arg) {
    ESP_LOGI(TAG, "IPv6 timer callback triggered"); 
    WifiBoard* board = static_cast<WifiBoard*>(arg);
    board->CheckIPv6Address();
}

void WifiBoard::StartIPv6Monitoring() {
     ESP_LOGI(TAG, "StartIPv6Monitoring called, timer=%p, obtained=%d", 
             ipv6_check_timer_, ipv6_global_obtained_);  // 添加调试
    if (ipv6_check_timer_ && !ipv6_global_obtained_) {
        ESP_LOGI(TAG, "Starting IPv6 monitoring (check every 3 seconds)");
        esp_err_t ret = esp_timer_start_periodic(ipv6_check_timer_, 3000000);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to start IPv6 timer: %s", esp_err_to_name(ret));
        }
    } else {
        ESP_LOGW(TAG, "Cannot start IPv6 monitoring: timer=%p, obtained=%d", 
                 ipv6_check_timer_, ipv6_global_obtained_);
    }
}

void WifiBoard::StopIPv6Monitoring() {
    if (ipv6_check_timer_) {
        esp_timer_stop(ipv6_check_timer_);
        ESP_LOGI(TAG, "IPv6 monitoring stopped");
    }
}

void WifiBoard::PrintIPv6Address(esp_ip6_addr_t& ip6_addr, const char* type) {
    ...
    ESP_LOGI(TAG, "IPv6 %s address: %s", type, ip6_str);
}

void WifiBoard::CheckIPv6Address() {
    ESP_LOGI(TAG, "CheckIPv6Address called"); 
    esp_netif_t* netif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    if (!netif) {
        ESP_LOGW(TAG, "WiFi station netif not found");
        return;
    ...
    // 检查链路本地地址
    if (esp_netif_get_ip6_linklocal(netif, &ip6_addr) == ESP_OK) {
        if (!ipv6_link_local_printed_) {
            PrintIPv6Address(ip6_addr, "link-local");
            ipv6_link_local_printed_ = true;
        }
    }
    
    // 检查全局地址
    if (esp_netif_get_ip6_global(netif, &ip6_addr) == ESP_OK) {
        if (!ipv6_global_obtained_) {
            PrintIPv6Address(ip6_addr, "global");
            ipv6_global_obtained_ = true;
            StopIPv6Monitoring();
        }
    } else {
        if (!ipv6_global_obtained_) {
            ESP_LOGD(TAG, "Waiting for IPv6 global address...");
            // 可选：定期尝试重新创建链路本地地址
            static int retry_count = 0;
            retry_count++;
            if (retry_count % 10 == 0) {  // 每 10 次检查（30秒）尝试一次
                esp_netif_create_ip6_linklocal(netif);
                ESP_LOGI(TAG, "Re-requesting IPv6 address");
            }
        }
    }
}