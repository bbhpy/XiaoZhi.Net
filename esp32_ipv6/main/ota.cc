...
esp_err_t Ota::CheckVersion() {
    ...
    if (cJSON_IsObject(mqtt)) {
        ...
        // 添加IPv6支持标志
        cJSON *ipv6 = cJSON_GetObjectItem(mqtt, "ipv6");
        if (cJSON_IsBool(ipv6)) {
            settings.SetBool("ipv6", ipv6->valueint);
            ESP_LOGI(TAG, "MQTT IPv6 support: %s", ipv6->valueint ? "enabled" : "disabled");
        }
    ...
    if (cJSON_IsObject(websocket)) {
        ...
        // 添加IPv6支持标志
        cJSON *ipv6 = cJSON_GetObjectItem(websocket, "ipv6");
        if (cJSON_IsBool(ipv6)) {
            settings.SetBool("ipv6", ipv6->valueint);
            ESP_LOGI(TAG, "WebSocket IPv6 support: %s", ipv6->valueint ? "enabled" : "disabled");
        }
