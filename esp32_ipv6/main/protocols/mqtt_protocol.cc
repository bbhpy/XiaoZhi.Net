...
static bool IsIPv6Address(const std::string& host) {
    ...
    // 简单检查：如果包含冒号且不是以[开头，则是纯IPv6地址
    return host.front() != '[';
}

/**
 * 从可能带方括号的地址中提取纯IP地址
 * 输入：[2001:db8::1] 或 2001:db8::1 或 192.168.1.1
 */
static std::string ExtractIpAddress(const std::string& address) {
...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    mqtt_->OnMessage([this](const std::string& topic, const std::string& payload) {
        ...
        // 新增：打印收到的MQTT消息（topic + payload）
        //ESP_LOGI(TAG, "[MQTT接收] topic: %s, 内容: %s", topic.c_str(), payload.c_str());
    ...
    // 检查是否是IPv6地址格式（带方括号）
    if (endpoint.front() == '[') {
        // IPv6格式：[2001:db8::1]:8883
        size_t bracket_end = endpoint.find(']');
        if (bracket_end != std::string::npos) {
            broker_address = endpoint.substr(1, bracket_end - 1);
            // 查找端口
            if (endpoint.length() > bracket_end + 1 && endpoint[bracket_end + 1] == ':') {
                broker_port = std::stoi(endpoint.substr(bracket_end + 2));
            }
        } else {
            ESP_LOGE(TAG, "Invalid IPv6 endpoint format: %s", endpoint.c_str());
            SetError(Lang::Strings::SERVER_NOT_FOUND);
            return false;
        }
    ...
    ESP_LOGI(TAG, "MQTT connecting to %s:%d", broker_address.c_str(), broker_port);
...
bool MqttProtocol::SendText(const std::string& text) {
    ...
    // 新增：打印发送的MQTT消息内容
    //ESP_LOGI(TAG, "[MQTT发送] topic: %s, 内容: %s", publish_topic_.c_str(), text.c_str());
    return true;
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    // ========== 修改：使用提取后的IPv6地址 ==========
    std::string udp_addr = ExtractIpAddress(udp_server_);
    ESP_LOGI(TAG, "UDP connecting to %s:%d", udp_addr.c_str(), udp_port_);

    udp_->Connect(udp_addr, udp_port_);
...
void MqttProtocol::ParseServerHello(const cJSON* root) {
    ...
    udp_port_ = cJSON_GetObjectItem(udp, "port")->valueint;
        // ========== 新增：UDP IPv6地址处理 ==========
    std::string udp_addr = ExtractIpAddress(udp_server_);
    ESP_LOGI(TAG, "UDP server: %s, port: %d (original: %s)", 
             udp_addr.c_str(), udp_port_, udp_server_.c_str());
    ...
    ESP_LOGI(TAG, "Received server assigned SSRC: %u", server_udp_ssrc_);
