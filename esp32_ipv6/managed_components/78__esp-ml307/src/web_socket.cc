...
bool WebSocket::Connect(const char* uri) {
    ...
    if (uri_str[pos] == '[') {
        ...
        // 提取纯 IPv6 地址，不带方括号
        host = uri_str.substr(pos + 1, next_pos - pos - 1);
        pos = next_pos + 1;
    ...
    connected_ = false;
    ESP_LOGI(TAG, "Connecting with host=[%s], port=%s", host.c_str(), port.c_str());
    // 使用 tcp 建立连接
   ...
   std::string request = "GET " + path + " HTTP/1.1\r\n";
    if (headers_.find("Host") == headers_.end()) {
        ...
        if (host_header.front() == '[' && host_header.back() == ']') {
            host_header = host_header.substr(1, host_header.length() - 2);
        }
